...
author: "Paulo Inácio Prado, Alexandre Oliveira, André Chalom"
date: "June 2016"
output:
...
supporting roots and the area at groud level encompassed by these roots.
...
This pattern suggests that the relationship between torque and number os roots differ between the two sites.

### Shuffling rows within the strata

#### Null hypothesis

In order to illustrate how to run ranodmization restricted to strata we will test
...
```{r rhyzophora statistcs of interest}
...
Indee it seems that this regression underestimates the number of roots of
...
```{r plot rhyzophora ancova, fig.cap = "Relation between number of supporting roots and the ratio canopy area / trunk area for mangrove trees at two sites that differ in the soil instability (medium and high instability). Also shown the line sof the regressions fitted to each site, but with a common slope."}
...
[^3]: Technical detail: we add the term `-1` to the regression fromula in order to
